...
void Processor_InitializeRegisters(int regPC, int regAcum,
                                   unsigned int regPSW) {
...
void Processor_FetchInstruction() {
  ...
  // The instruction must be located at the memory address pointed by the PC
  // register
  registerMAR_CPU = registerPC_CPU;
  // Send to the main memory controller the address in which the reading has to
  // take place: use the address bus for this
...
void Processor_DecodeAndExecuteInstruction() {
  ...
  // Change to contain the PSW value in red along with its tag.
