...
unsigned int registerPSW_CPU =
    128;                    // Processor state word, initially protected mode
...
// interrupt vector table: an array of handle interrupt memory addresses
// routines
...
char *PSW_BITS_names[INTERRUPTTYPES] = {
    "POWEROFF_BIT", "ZERO_BIT", "NEGATIVE_BIT",       "OVERFLOW_BIT", "UNUSED",
    "UNUSED",       "UNUSED",   "EXECUTION_MODE_BIT", "UNUSED",       "UNUSED"};

// Initialization of the interrupt vector table
void Processor_InitializeInterruptVectorTable(
    int interruptVectorInitialAddress) {
  int i;
...
void Processor_FetchInstruction() {
  ...
  // The instruction must be located at the logical memory address pointed by
  // the PC register
  registerMAR_CPU = registerPC_CPU;
  // Send to the MMU the address in which the reading has to take place: use the
  // address bus for this
  Buses_write_AddressBus_From_To(CPU, MMU);
  // Tell the main memory controller to read
  if (MMU_readMemory() == MMU_SUCCESS) {
    // All the read data is stored in the MBR register. Because it is an
    // instruction we have to copy it to the IR register
    ...
    ComputerSystem_DebugMessage(1, HARDWARE, registerIR_CPU.operationCode,
                                registerIR_CPU.operand1,
                                registerIR_CPU.operand2);
  } else
    // Show message: "_ _ _ "
    Processor_ShowTime(HARDWARE);
...
void Processor_ShowTime(char section) {
  ComputerSystem_DebugMessage(
      98, section, Processor_PSW_BitState(EXECUTION_MODE_BIT) ? "\t" : "");
  ComputerSystem_DebugMessage(Processor_PSW_BitState(EXECUTION_MODE_BIT) ? 5
                                                                         : 4,
...
void Processor_DecodeAndExecuteInstruction() {
  ...
  // Instruction WRITE
  case 'w':
    registerMBR_CPU.operationCode = registerMBR_CPU.operand1 =
        registerMBR_CPU.operand2 = registerAccumulator_CPU;
    registerMAR_CPU = registerIR_CPU.operand1;
    // Send to the main memory controller the data to be written: use the data
    // bus for this
    Buses_write_DataBus_From_To(CPU, MAINMEMORY);
    // Send to the main memory controller the address in which the writing has
    // to take place: use the address bus for this
    Buses_write_AddressBus_From_To(CPU, MMU);
  ...
  case 'r':
    registerMAR_CPU = registerIR_CPU.operand1;
    // Send to the main memory controller the address in which the reading has
    // to take place: use the address bus for this
    Buses_write_AddressBus_From_To(CPU, MMU);
    // Tell the main memory controller to read
    MMU_readMemory();
  ...
  // Instruction OS
  case 'o': // Make a operating system routine in entry point indicated by
            // operand1
    if (Processor_PSW_BitState(EXECUTION_MODE_BIT)) {
      // Show final part of HARDWARE message with CPU registers
      // Show message: " (PC: registerPC_CPU, Accumulator:
      // registerAccumulator_CPU, PSW: registerPSW_CPU [Processor_ShowPSW()]\n
      Processor_ShowTime(HARDWARE);
      ComputerSystem_DebugMessage(3, HARDWARE, registerPC_CPU,
                                  registerAccumulator_CPU, registerPSW_CPU,
                                  Processor_ShowPSW());
      // Not all operating system code is executed in simulated processor, but
      // really must do it...
      OperatingSystem_InterruptLogic(registerIR_CPU.operand1);
      registerPC_CPU++;
      // Update PSW bits (ZERO_BIT, NEGATIVE_BIT, ...)
      Processor_UpdatePSW();
      return; // Note: message show before... for operating system messages
              // after...
    } else {
      Processor_RaiseInterrupt(EXCEPTION_BIT);
    }
  ...
  // Show final part of HARDWARE message with	CPU registers
  // Show message: " (PC: registerPC_CPU, Accumulator: registerAccumulator_CPU,
  // PSW: registerPSW_CPU [Processor_ShowPSW()]\n
  Processor_ShowTime(HARDWARE);
  ComputerSystem_DebugMessage(3, HARDWARE, registerPC_CPU,
                              registerAccumulator_CPU, registerPSW_CPU,
                              Processor_ShowPSW());
...
unsigned int
Processor_GetInterruptLineStatus(const unsigned int interruptNumber) {
  unsigned int mask = 1;
...
int Processor_GetMAR() { return registerMAR_CPU; }

// Setter for the registerMAR_CPU
void Processor_SetMAR(int data) { registerMAR_CPU = data; }
...
int Processor_GetMBR_Value() { return registerMBR_CPU.operationCode; }

// Setter for the Accumulator
void Processor_SetAccumulator(int acc) { registerAccumulator_CPU = acc; }

// Setter for the PC
void Processor_SetPC(int pc) { registerPC_CPU = pc; }
...
int Processor_GetAccumulator() { return registerAccumulator_CPU; }

// int Processor_GetPC() {
//   return registerPC_CPU;
// }

int Processor_GetRegisterA() { return registerA_CPU; }

// Setter for the PSW
void Processor_SetPSW(unsigned int psw) { registerPSW_CPU = psw; }

// Getter for the PSW
unsigned int Processor_GetPSW() { return registerPSW_CPU; }
