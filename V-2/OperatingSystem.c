...
// Initially, it creates a process from each program specified in the command
// line
int OperatingSystem_LongTermScheduler() {
  ...
  for (i = 0; userProgramsList[i] != NULL && i < USERPROGRAMSMAXNUMBER; i++) {
    ...
    // Si el PID es igual a NOFREENTRY decrementamos el
    // numberOfSuccessfullyCreatedProcesses porque  el proceso no se ha creado
    // correctamente
    if (PID == NOFREEENTRY) {
      ComputerSystem_DebugMessage(103, ERROR,
      ...
      ComputerSystem_DebugMessage(105, ERROR,
                                  userProgramsList[i]->executableName);
      numberOfSuccessfullyCreatedProcesses--;
    } else {
...
int OperatingSystem_CreateProcess(USER_PROGRAMS_DATA executableProgram) {
  ...
  // Si OperatingSystem_ObtainAnEntryInTheProcessTable() fracasa devolvemos
  // NOFREENTRY
  if (PID == NOFREEENTRY) {
    return PID;
  }
  ...
  int isBig = loadingPhysicalAddress =
      OperatingSystem_ObtainMainMemory(processSize, PID);
  if (isBig == TOOBIGPROCESS) {
    return isBig;
  }
  // Load program in the allocated memory
  int big = OperatingSystem_LoadProgram(programFile, loadingPhysicalAddress,
                                        processSize);
...
void OperatingSystem_MoveToTheREADYState(int PID) {
  ...
  if (Heap_add(PID, readyToRunQueue[cola], QUEUE_PRIORITY,
               &numberOfReadyToRunProcesses[cola], PROCESSTABLEMAXSIZE) >= 0) {
...
int OperatingSystem_ExtractFromReadyToRun() {
  ...
  selectedProcess = Heap_poll(readyToRunQueue[cola], QUEUE_PRIORITY,
                              &numberOfReadyToRunProcesses[cola]);
...
void OperatingSystem_Dispatch(int PID) {
  ...
  ComputerSystem_DebugMessage(
      ...
      statesNames[processTable[executingProcessID].state], statesNames[2]);
...
void OperatingSystem_HandleSystemCall() {
  ...
  case SYSCALL_YIELD:
    if (processTable[processToRun].priority ==
        processTable[executingProcessID].priority) {
      OperatingSystem_PreemptRunningProcess();
      OperatingSystem_Dispatch(OperatingSystem_ShortTermScheduler());
      ComputerSystem_DebugMessage(115, SHORTTERMSCHEDULE, oldProcess,
                                  processToRun);
    }
    break;
  }
