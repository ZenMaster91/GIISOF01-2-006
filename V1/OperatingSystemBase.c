...
int OperatingSystem_LoadProgram(FILE *programFile, int initialAddress,
  ...
  while (fgets(lineRead, MAXLINELENGTH, programFile) != NULL) {
    // REMARK: if lineRead is greater than MAXLINELENGTH in number of
    // characters, the program loading does not work
    data.operationCode = ' ';
...
void OperatingSystem_ReadyToShutdown() {
  // Simulation must finish (done by modifying the PC of the System Idle Process
  // so it points to its 'halt' instruction, located at the last memory position
  // used by that process, and dispatching sipId (next ShortTermSheduled)
